"""
Fast parallel improvement test — 3 key scenarios only.
"""
import pickle, numpy as np, pandas as pd, time, multiprocessing
...
def run_sim(args):
    pdata, corr_matrix, tf_enabled, mr_risk, exclude_pairs, exclude_hours, sessions, label = args
    ...
    for i in range(250, n_bars-1):
        ...
        for pair in active_pairs:
            ...
            if abs(dist_from_ema) < BREAKOUT_ZONE:
                ...
                open_pos.append({'pair':pair,'trend':-1,'entry':entry_fill,'sl':sl,'tp':tp,'lot':sz.lot_size,'bar':i,'pip':pip,'pv':pd_['pv'],'spread':pd_['spread'],'regime':'tf'})
                n_tf+=1
    if not oos_pnls or oos_bal_start is None: return (label, None)
    ...
    return (label, {
        'oos_n':len(oos_pnls), 'oos_wr':ow/(ow+ol)*100, 'oos_pf':ogw/ogl if ogl>0 else 99,
        'oos_mdd':oos_mdd*100, 'oos_bal_start':oos_bal_start, 'oos_bal_end':oos_bal_end,
        'oos_avg':np.mean(oos_pnls), 'oos_avg_win':np.mean([p for p in oos_pnls if p>0]),
        'oos_avg_loss':np.mean([p for p in oos_pnls if p<=0]) if ol>0 else 0,
        'full_n':len(pnls), 'full_wr':fw/(fw+fl)*100,
        'full_pf':fgw/fgl if fgl>0 else 99, 'full_mdd':mdd*100,
        'full_final':bal, 'full_avg':np.mean(pnls),
    })

if __name__=='__main__':
    print("="*80)
    print("  PARALLEL IMPROVEMENT TEST — OOS (2025-2026)")
    print("="*80, flush=True)
    ...
    scenarios = [
        ("0: Baseline (MR+TF, Lon+NY)", False, MR_RISK, set(), set(), SESSIONS),
        ("1: MR Only, Lon+NY", False, 0.035, set(), set(), SESSIONS),
        ("2: MR Only, London", False, 0.035, set(), set(), {'london':(7,16)}),
        ("3: MR Only, London, 5%", False, 0.05, set(), set(), {'london':(7,16)}),
        ("4: MR Only, London, NoWeak", False, 0.035, {'AUD/CAD','NZD/USD','AUD/JPY'}, set(), {'london':(7,16)}),
        ("5: MR Only, London, 5%, NoWeak", False, 0.05, {'AUD/CAD','NZD/USD','AUD/JPY'}, set(), {'london':(7,16)}),
    ]

    args_list = [(pdata, corr_matrix, tf, mr, ex_p, ex_h, sess, label) for label, tf, mr, ex_p, ex_h, sess in scenarios]

    print(f"\n  Running {len(scenarios)} scenarios in parallel...", flush=True)
    t0=time.time()
    with multiprocessing.Pool(len(scenarios)) as pool:
        results = pool.map(run_sim, args_list)
    ...
    print("="*80)
    print(f"\n  {'Scenario':<35s} {'Trades':>6s} {'WR%':>6s} {'PF':>5s} {'MDD%':>6s} {'AvgP&L':>9s} {'$/mo':>10s} {'BalEnd':>12s}")
    print(f"  {'-'*35} {'-'*6} {'-'*6} {'-'*5} {'-'*6} {'-'*9} {'-'*10} {'-'*12}")
    for label, r in results:
        if r is None:
            print(f"  {label:<35s} {'NONE':>6s}")
        else:
            monthly = r['oos_avg'] * r['oos_n'] / 18.5
            print(f"  {label:<35s} {r['oos_n']:>6d} {r['oos_wr']:>5.1f}% {r['oos_pf']:>5.1f} {r['oos_mdd']:>5.2f}% ${r['oos_avg']:>8,.0f} ${monthly:>9,.0f} ${r['oos_bal_end']:>11,.0f}")

    print("\n" + "="*80)
    print("  FULL PERIOD (2022-2026)")
    print("="*80)
    print(f"\n  {'Scenario':<35s} {'Trades':>6s} {'WR%':>6s} {'PF':>5s} {'MDD%':>6s} {'AvgP&L':>9s} {'Final$':>12s}")
    print(f"  {'-'*35} {'-'*6} {'-'*6} {'-'*5} {'-'*6} {'-'*9} {'-'*12}")
    for label, r in results:
        if r is None:
            print(f"  {label:<35s} {'NONE':>6s}")
        else:
            print(f"  {label:<35s} {r['full_n']:>6d} {r['full_wr']:>5.1f}% {r['full_pf']:>5.1f} {r['full_mdd']:>5.2f}% ${r['full_avg']:>8,.0f} ${r['full_final']:>11,.0f}")
