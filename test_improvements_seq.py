"""
Sequential improvement test — 3 key scenarios only.
"""
...
def run_sim(pdata, corr_matrix, tf_enabled=True, mr_risk=MR_RISK, exclude_pairs=None, exclude_hours=None, sessions=None):
    ...
    for i in range(250, n_bars-1):
        ...
        for pair in active_pairs:
            ...
            if abs(dist_from_ema) < BREAKOUT_ZONE:
                ...
                if not sz.ok: continue
                open_pos.append({'pair':pair,'trend':-1,'entry':entry_fill,'sl':sl,'tp':tp,'lot':sz.lot_size,'bar':i,'pip':pip,'pv':pd_['pv'],'spread':pd_['spread'],'regime':'tf'})
    if not oos_pnls or oos_bal_start is None: return None
    ...
    return {
        ...
        'oos_avg':np.mean(oos_pnls), 'oos_avg_win':np.mean([p for p in oos_pnls if p>0]) if ow>0 else 0,
        'oos_avg_loss':np.mean([p for p in oos_pnls if p<=0]) if ol>0 else 0,
        'oos_tpm':len(oos_pnls)/oos_nm,
        'full_n':len(pnls), 'full_wr':fw/(fw+fl)*100,
        'full_pf':fgw/fgl if fgl>0 else 99, 'full_mdd':mdd*100,
        'full_final':bal, 'full_avg':np.mean(pnls), 'full_tpm':len(pnls)/fnm,
    }

print("="*80)
print("  IMPROVEMENT TEST — OOS (2025-2026)")
print("="*80, flush=True)
...
scenarios = [
    ...
    ("3: MR Only, London, 5%", dict(tf_enabled=False, mr_risk=0.05, sessions={'london':(7,16)})),
]

for name, params in scenarios:
    print(f"\n  {name}...", end='', flush=True)
    t0=time.time()
    r = run_sim(pdata, corr_matrix, **params)
    elapsed=time.time()-t0
    if r:
        print(f" {elapsed:.0f}s", flush=True)
        m = r['oos_avg'] * r['oos_tpm']
        print(f"    OOS: {r['oos_n']}t WR={r['oos_wr']:.1f}% PF={r['oos_pf']:.1f} MDD={r['oos_mdd']:.2f}% AvgP&L=${r['oos_avg']:,.0f} AvgWin=${r['oos_avg_win']:,.0f} AvgLoss=${r['oos_avg_loss']:,.0f} $/mo=${m:,.0f}")
        print(f"    FULL: {r['full_n']}t WR={r['full_wr']:.1f}% PF={r['full_pf']:.1f} MDD={r['full_mdd']:.2f}% Final=${r['full_final']:,.0f}")
    else:
