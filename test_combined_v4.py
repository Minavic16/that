"""
Backtest v4: Proven MR + Order Flow Entry Filter
Keep the proven MR logic from test_final_scalper.py, add order flow as confirmation.
"""
import pandas as pd
import numpy as np
import pickle
...
# Session times (UTC)
LONDON_OPEN, LONDON_CLOSE = 7, 16
NY_OPEN, NY_CLOSE = 12, 21
...
def compute_order_imbalance(df, window=10):
    ...
    for i in range(window, n):
        order_imb[i] = (cum_signed[i] - cum_signed[i-window]) / (cum_vol[i] - cum_vol[i-window] + 1e-10)
    return order_imb
...
def run_backtest(pairs, start_date='2025-01-01', end_date='2026-07-15'):
    ...
    for pair_file, pair_name in pairs:
        ...
        pair_ind[pair_name] = {
            ...
            'order_imb': compute_order_imbalance(df, 10),
        }
    ...
    for t in all_times:
        ...
        for pn, df in pair_data.items():
            ...
            order_imb = pair_ind[pn]['order_imb'][idx]

            if np.isnan(atr) or atr == 0:
                continue

            # LONG: RSI oversold + price below both EMAs + order imbalance positive
            if (rsi_val < 30 and
                price < ema_f and price < ema_s and
                order_imb > 0.05):
            ...
            # SHORT: RSI overbought + price above both EMAs + order imbalance negative
            elif (rsi_val > 70 and
                  price > ema_f and price > ema_s and
                  order_imb < -0.05):
...
if __name__ == '__main__':
    ...
    print("=" * 70)
    print("BACKTEST v4: Proven MR + Order Flow Filter")
    print("=" * 70)
