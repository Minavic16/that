"""
Combined Backtest v3 - Scoring System
Three-layer scoring:
  1. COT: 0-3 points (institutional positioning)
  2. Order Flow: 0-3 points (microstructure)
  3. MR: 0-3 points (mean reversion)
Entry when score >= 5, exit at session close or stop loss.
"""
import pandas as pd
import numpy as np
import pickle
import os
from typing import Dict, List, Tuple
from collections import defaultdict

from cot_signal import COTSignal

# =================== CONFIG ===================
RISK_PER_TRADE = 0.01
MAX_CONCURRENT = 5
DAILY_LOSS_LIMIT = 0.03
LEVERAGE = 100
INITIAL_CAPITAL = 2500.0
MIN_SCORE = 8
COOLDOWN_BARS = 8  # 4 hours between trades on same pair
MIN_ATR_MULT = 0.5  # Only trade when ATR > 0.5 * average ATR

SPREAD_MODEL = {
    'EUR/USD': 0.8, 'GBP/USD': 1.0, 'USD/JPY': 1.0, 'AUD/USD': 1.2,
    'USD/CAD': 1.2, 'USD/CHF': 1.2, 'GBP/JPY': 3.0, 'EUR/JPY': 2.5,
    'AUD/JPY': 2.5, 'NZD/USD': 1.5,
}


def pip_info(pair_name):
    pip_size = 0.01 if 'JPY' in pair_name else 0.0001
    return pip_size, 10.0


def load_pair_data(pair_file, pair_name):
    fpath = os.path.join('/root/data', f'{pair_file}.pkl')
    if not os.path.exists(fpath):
        return pd.DataFrame()
    data = pickle.load(open(fpath, 'rb'))
    df = data[pair_name].copy()
    df.index = pd.to_datetime(df.index)
    if df.index.tz is not None:
        df.index = df.index.tz_localize(None)
    if len(df) > 100000:
        df = df.resample('30min').agg({
            'open': 'first', 'high': 'max', 'low': 'min',
            'close': 'last', 'volume': 'sum'
        }).dropna()
    return df


def compute_rsi(close, period=14):
    delta = close.diff()
    gain = delta.where(delta > 0, 0.0)
    loss = -delta.where(delta < 0, 0.0)
    avg_gain = gain.ewm(alpha=1/period, min_periods=period).mean()
    avg_loss = loss.ewm(alpha=1/period, min_periods=period).mean()
    rs = avg_gain / (avg_loss + 1e-10)
    return 100 - (100 / (1 + rs))


def compute_ema(close, period):
    return close.ewm(span=period, adjust=False).mean()


def precompute_of(df):
    """Lightweight order flow: tick rule + VWAP + momentum."""
    n = len(df)
    close = df['close'].values
    vol = df['volume'].values

    tick_sign = np.zeros(n)
    for i in range(1, n):
        if close[i] > close[i-1]:
            tick_sign[i] = 1.0
        elif close[i] < close[i-1]:
            tick_sign[i] = -1.0
        else:
            tick_sign[i] = tick_sign[i-1]

    signed_vol = tick_sign * vol

    # 10-bar rolling order imbalance
    order_imb = np.zeros(n)
    cum_signed = np.cumsum(signed_vol)
    cum_vol = np.cumsum(vol)
    for i in range(10, n):
        order_imb[i] = (cum_signed[i] - cum_signed[i-10]) / (cum_vol[i] - cum_vol[i-10] + 1e-10)

    # 20-bar rolling VWAP deviation
    cum_pv = np.cumsum(close * vol)
    vwap = cum_pv / (cum_vol + 1e-10)
    vwap_dev = (close - vwap) / (vwap + 1e-10)

    # 5-bar momentum
    momentum = np.zeros(n)
    for i in range(5, n):
        momentum[i] = (close[i] - close[i-5]) / (close[i-5] + 1e-10)

    return order_imb, vwap_dev, momentum


def run_backtest(pairs, start_date='2025-01-01', end_date='2026-07-15'):
    cot = COTSignal()
    capital = INITIAL_CAPITAL
    daily_pnl = defaultdict(float)
    current_date = None
    open_positions = []
    all_trades = []
    equity_curve = []
    last_trade_idx = defaultdict(int)  # Cooldown tracker

    start_dt = pd.Timestamp(start_date)
    end_dt = pd.Timestamp(end_date)

    # Preload
    pair_data = {}
    pair_of = {}
    pair_ind = {}
    cot_signals = {}

    for pair_file, pair_name in pairs:
        df = load_pair_data(pair_file, pair_name)
        if len(df) == 0:
            continue
        df = df[(df.index >= start_dt) & (df.index <= end_dt)]
        if len(df) == 0:
            continue
        pair_data[pair_name] = df
        print(f"Loaded {pair_name}: {len(df)} bars")

        # Precompute indicators
        rsi = compute_rsi(df['close'])
        ema_f = compute_ema(df['close'], 8)
        ema_s = compute_ema(df['close'], 21)
        order_imb, vwap_dev, momentum = precompute_of(df)
        pair_ind[pair_name] = {
            'rsi': rsi, 'ema_fast': ema_f, 'ema_slow': ema_s,
            'atr': compute_atr(df, 14),
        }
        pair_of[pair_name] = {
            'order_imb': order_imb, 'vwap_dev': vwap_dev, 'momentum': momentum,
        }
        cot_signals[pair_name] = cot.get_signal(pair_name)

    if not pair_data:
        print("No data!")
        return {}

    for pn, cs in cot_signals.items():
        print(f"  COT {pn}: {cs['direction']} ({cs['strength']:.2f})")

    all_times = sorted(set().union(*[set(df.index) for df in pair_data.values()]))
    print(f"\nBacktest: {all_times[0]} to {all_times[-1]}, {len(all_times)} bars\n")

    for t in all_times:
        nd = t.date()
        if current_date != nd:
            daily_pnl = defaultdict(float)
            current_date = nd

        # --- EXITS ---
        remaining = []
        for pos in open_positions:
            pn = pos['pair']
            df = pair_data.get(pn)
            if df is None or t not in df.index:
                remaining.append(pos)
                continue
            price = df.loc[t, 'close']
            pip_size, pip_dollar = pip_info(pn)

            if t.hour == 17:
                should_exit, reason = True, 'session_close'
            elif pos['direction'] == 'long' and price <= pos['sl']:
                should_exit, reason = True, 'stop_loss'
            elif pos['direction'] == 'short' and price >= pos['sl']:
                should_exit, reason = True, 'stop_loss'
            else:
                should_exit = False

            if should_exit:
                spread = SPREAD_MODEL.get(pn, 1.5) * pip_size
                if pos['direction'] == 'long':
                    exit_p = price - spread / 2
                    pnl_pips = (exit_p - pos['entry']) / pip_size
                else:
                    exit_p = price + spread / 2
                    pnl_pips = (pos['entry'] - exit_p) / pip_size
                pnl_pips -= 1.4
                pnl_dollars = pnl_pips * pip_dollar * pos['lots']

                daily_pnl[nd] += pnl_dollars
                capital += pnl_dollars
                all_trades.append({
                    'pair': pn, 'dir': pos['direction'],
                    'entry_time': pos['entry_time'], 'exit_time': t,
                    'pnl_pips': pnl_pips, 'pnl$': pnl_dollars, 'reason': reason,
                })
            else:
                remaining.append(pos)
        open_positions = remaining

        if daily_pnl[nd] < -(DAILY_LOSS_LIMIT * capital):
            continue
        if len(open_positions) >= MAX_CONCURRENT:
            continue

        # --- ENTRIES ---
        for pn, df in pair_data.items():
            if t not in df.index:
                continue
            idx = df.index.get_loc(t)
            if idx < 50:
                continue

            # Session filter: only trade 8:00-17:00 EST (13:00-02:00 UTC)
            hour_utc = t.hour
            if not (13 <= hour_utc or hour_utc <= 2):
                continue

            # Cooldown filter
            if idx - last_trade_idx[pn] < COOLDOWN_BARS:
                continue

            # ATR volatility filter
            atr_val = pair_ind[pn]['atr'].iloc[idx]
            atr_avg = pair_ind[pn]['atr'].iloc[max(0, idx-200):idx].mean()
            if atr_val < atr_avg * MIN_ATR_MULT:
                continue

            price = df.loc[t, 'close']
            pip_size, pip_dollar = pip_info(pn)
            spread = SPREAD_MODEL.get(pn, 1.5) * pip_size

            rsi_val = pair_ind[pn]['rsi'].iloc[idx]
            ema_f = pair_ind[pn]['ema_fast'].iloc[idx]
            atr = pair_ind[pn]['atr'].iloc[idx]

            order_imb = pair_of[pn]['order_imb'][idx]
            vwap_dev = pair_of[pn]['vwap_dev'][idx]
            momentum = pair_of[pn]['momentum'][idx]

            cs = cot_signals[pn]

            # --- SCORING ---
            for direction in ['long', 'short']:
                score = 0

                # COT layer (0-3) — optional bonus, not required
                if direction == 'long':
                    if cs['direction'] == 'long':
                        score += min(int(cs['strength'] * 2), 2)
                    elif cs['direction'] == 'short':
                        score -= 1
                else:
                    if cs['direction'] == 'short':
                        score += min(int(cs['strength'] * 2), 2)
                    elif cs['direction'] == 'long':
                        score -= 1

                # Order Flow layer (0-4)
                if direction == 'long':
                    if order_imb > 0.15:
                        score += 3
                    elif order_imb > 0.05:
                        score += 2
                    elif order_imb > 0.0:
                        score += 1
                    if vwap_dev < -0.001:
                        score += 1
                else:
                    if order_imb < -0.15:
                        score += 3
                    elif order_imb < -0.05:
                        score += 2
                    elif order_imb < 0.0:
                        score += 1
                    if vwap_dev > 0.001:
                        score += 1

                # MR layer (0-4)
                if direction == 'long':
                    if rsi_val < 25:
                        score += 4
                    elif rsi_val < 30:
                        score += 3
                    elif rsi_val < 35:
                        score += 2
                    elif rsi_val < 45:
                        score += 1
                    if price < ema_f:
                        score += 1
                else:
                    if rsi_val > 75:
                        score += 4
                    elif rsi_val > 70:
                        score += 3
                    elif rsi_val > 65:
                        score += 2
                    elif rsi_val > 55:
                        score += 1
                    if price > ema_f:
                        score += 1

                # Momentum filter: fade against strong momentum
                if direction == 'long' and momentum > 0.003:
                    score -= 2  # Don't buy into strong rally
                if direction == 'short' and momentum < -0.003:
                    score -= 2  # Don't sell into strong drop

                if score < MIN_SCORE:
                    continue

                # Entry
                if direction == 'long':
                    entry_p = price + spread / 2
                    sl = entry_p - 2 * atr if atr > 0 else entry_p - 20 * pip_size
                else:
                    entry_p = price - spread / 2
                    sl = entry_p + 2 * atr if atr > 0 else entry_p + 20 * pip_size

                risk_amt = capital * RISK_PER_TRADE
                sl_dist = abs(entry_p - sl)
                if sl_dist == 0:
                    continue
                lots = round(risk_amt / (sl_dist / pip_size * pip_dollar), 2)
                if lots < 0.01:
                    continue
                margin = lots * 100000 * entry_p / LEVERAGE
                if margin > capital * 0.95:
                    continue

                open_positions.append({
                    'pair': pn, 'direction': direction,
                    'entry': entry_p, 'entry_time': t,
                    'sl': sl, 'lots': lots, 'score': score,
                })
                last_trade_idx[pn] = idx

        equity_curve.append({'time': t, 'equity': capital})

    if not all_trades:
        print("No trades!")
        return {}

    wins = [t for t in all_trades if t['pnl$'] > 0]
    losses = [t for t in all_trades if t['pnl$'] <= 0]
    total_pnl = sum(t['pnl$'] for t in all_trades)
    wr = len(wins) / len(all_trades) * 100
    avg_win = np.mean([t['pnl$'] for t in wins]) if wins else 0
    avg_loss = abs(np.mean([t['pnl$'] for t in losses])) if losses else 0
    pf = sum(t['pnl$'] for t in wins) / (sum(abs(t['pnl$']) for t in losses) + 1e-10)

    peak_eq = INITIAL_CAPITAL
    max_dd = 0
    for eq in equity_curve:
        if eq['equity'] > peak_eq:
            peak_eq = eq['equity']
        dd = (peak_eq - eq['equity']) / peak_eq * 100
        if dd > max_dd:
            max_dd = dd

    pair_stats = defaultdict(lambda: {'trades': 0, 'wins': 0, 'pnl': 0})
    for t in all_trades:
        pair_stats[t['pair']]['trades'] += 1
        pair_stats[t['pair']]['pnl'] += t['pnl$']
        if t['pnl$'] > 0:
            pair_stats[t['pair']]['wins'] += 1

    exit_stats = defaultdict(int)
    for t in all_trades:
        exit_stats[t['reason']] += 1

    months = (all_times[-1] - all_times[0]).days / 30.0
    cagr = ((capital / INITIAL_CAPITAL) ** (12 / months) - 1) * 100 if months > 0 else 0

    return {
        'total_trades': len(all_trades),
        'win_rate': wr,
        'total_pnl': total_pnl,
        'profit_factor': pf,
        'avg_win': avg_win,
        'avg_loss': avg_loss,
        'max_drawdown_pct': max_dd,
        'final_capital': capital,
        'cagr_monthly': cagr,
        'trades': all_trades,
        'equity_curve': equity_curve,
        'pair_stats': dict(pair_stats),
        'exit_stats': dict(exit_stats),
    }


def compute_atr(df, period=14):
    high = df['high']
    low = df['low']
    close = df['close']
    tr1 = high - low
    tr2 = abs(high - close.shift(1))
    tr3 = abs(low - close.shift(1))
    tr = pd.concat([tr1, tr2, tr3], axis=1).max(axis=1)
    return tr.rolling(period).mean()


if __name__ == '__main__':
    pairs = [
        ('EUR_USD', 'EUR/USD'), ('GBP_USD', 'GBP/USD'),
        ('USD_JPY', 'USD/JPY'), ('AUD_USD', 'AUD/USD'),
        ('USD_CAD', 'USD/CAD'), ('USD_CHF', 'USD/CHF'),
        ('GBP_JPY', 'GBP/JPY'), ('EUR_JPY', 'EUR/JPY'),
        ('AUD_JPY', 'AUD/JPY'), ('NZD_USD', 'NZD/USD'),
    ]

    print("=" * 70)
    print("COMBINED BACKTEST v3: COT + Order Flow + MR (Scoring)")
    print("=" * 70)

    results = run_backtest(pairs, start_date='2025-01-01', end_date='2026-07-15')

    if results:
        print(f"\n{'='*70}")
        print("RESULTS")
        print(f"{'='*70}")
        print(f"Total trades: {results['total_trades']}")
        print(f"Win rate: {results['win_rate']:.1f}%")
        print(f"Total P&L: ${results['total_pnl']:.2f}")
        print(f"Profit factor: {results['profit_factor']:.2f}")
        print(f"Avg win: ${results['avg_win']:.2f}")
        print(f"Avg loss: ${results['avg_loss']:.2f}")
        print(f"Max drawdown: {results['max_drawdown_pct']:.1f}%")
        print(f"Final capital: ${results['final_capital']:.2f}")
        print(f"CAGR (monthly): {results['cagr_monthly']:.1f}%")

        print(f"\nPer-pair stats:")
        for pair, stats in sorted(results['pair_stats'].items()):
            wr = stats['wins'] / stats['trades'] * 100 if stats['trades'] > 0 else 0
            print(f"  {pair}: {stats['trades']}T, {wr:.0f}% WR, ${stats['pnl']:.2f}")

        print(f"\nExit reasons:")
        for reason, count in sorted(results['exit_stats'].items()):
            print(f"  {reason}: {count}")
